...
func intro() {
	...
	fmt.Println("------------")
	fmt.Println("Enter a number, and we'll tell you if it is a prime number or not. Enter q to quit.")
	prompt()
}
...
func isPrime(n int) (bool, string) {
	...
	// use the moduls operator repeatedly to see if we have a prime number
	for i := 2; i <= n/2; i++ {
		if n%i == 0 {
			// not a prime number
